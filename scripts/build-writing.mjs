...
function markdownToHtml(markdown) {
  ...
  const flushParagraph = () => {
    if (paragraph.length) {
      const content = paragraph.map((line, index) => {
        const hardBreak = / {2,}$/.test(line);
        const separator = hardBreak ? "<br>" : index < paragraph.length - 1 ? " " : "";
        return `${renderInline(line.trim())}${separator}`;
      }).join("");
...
function normalizeArticle(data, body, file) {
  ...
  return {
    ...
    summary: String(data.summary || "").trim() || text.slice(0, 180) || "A new entry from Raphael Onuku.",
    image: String(data.image || "").trim(),
    image_alt: String(data.image_alt || "").trim(),
    social_image: String(data.social_image || "").trim(),
    body: body.trim(),
...
function publicAssetUrl(path, fallback = "/assets/social-preview-ro-20260910.png") {
